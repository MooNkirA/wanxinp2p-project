  ...
  <view class="content lendDetails">
    ...
    <view class="topHead">
      <view class="lendInfo">
        <view class="box">
          <view>
            <text class="num">{{ details.annualRate * 100 }}</text
            >%
          </view>
          <view class="des">约定借款年化利率</view>
          <view class="items">
      ...
      <view class="cardsBlock infoCont">
        <view class="tit"> 债券满30天可申请转让 </view>
    ...
    <view class="uni-tab-bar">
      <view id="tab-bar" class="uni-swiper-tab box-dis-flex">
        <view class="swiper-tab-list" :class="tabIndex == 0 ? 'active' : ''" @click="tapTab(0)"
          >原始借款人信息</view
        >
        <view class="swiper-tab-list" :class="tabIndex == 1 ? 'active' : ''" @click="tapTab(1)"
          >信用与认证</view
        >
        <view class="swiper-tab-list" :class="tabIndex == 2 ? 'active' : ''" @click="tapTab(2)"
          >收款计划</view
        >
      </view>
      <swiper :current="tabIndex" class="swiper-box" :duration="300" @change="changeTab">
        <swiper-item>
          <scroll-view class="cont" scroll-y>
            <view class="block">
              <view class="items"
                ><view>姓名</view> <view>{{ userInfo.fullname }}</view></view
              >
              <view class="items"
                ><view>年龄</view> <view>{{ userInfo.age }}</view></view
              >
              <view class="items"
                ><view>性别</view> <view>{{ userInfo.gender }}</view></view
              >
              <view class="items"
                ><view>出生年月</view> <view>{{ userInfo.birthday }}</view></view
              >
              <view class="items"
                ><view>手机号</view>
                <view>{{ utils.numReplaceStar(userInfo.mobile, 3, 7) }}</view></view
              >
              <view class="items"
                ><view>身份证</view>
                <view>{{ utils.numReplaceStar(userInfo.idNumber, 4, 14) }}</view></view
              >
        ...
        <swiper-item>
          <scroll-view class="cont" scroll-y>
            <view class="block">
              <view class="items"
                ><view>期次</view> <view>本金(元)</view><view>利息(元)</view>
                <view>状态</view></view
              >
              <view class="items" v-for="(item, index) in planData" :key="index">
                <view>{{ item.planItem }}期</view><view>{{ item.amount.toFixed(2) }}</view>
                <view>{{ item.interest.toFixed(2) }}</view> <view>{{ item.st }}</view>
              </view>
            </view>
          </scroll-view>
        </swiper-item>
    ...
    <view class="ButList" v-if="details.remainingAmount > 0">
      <view class="inpbox"
        ><input
          ...
          class="inpMonay"
          v-model="money"
          :placeholder="`金额不得高于${details.remainingAmount}`"
        />
        <text>元</text></view
      >
...
export default {
  ...
  computed: {
    proWid() {
      return `width:${
        ((this.details.amount - this.details.remainingAmount) / this.details.amount) * 100
      }%`
    },
  },
  components: {
